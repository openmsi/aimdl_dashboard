...
def _fetch_datafiles(data_type: str, total_limit: int) -> List[dict]:
    results: List[dict] = []
    offset = 0
    page_size = 100
...
def _build_viz(item: dict, data_type: str) -> Optional[dict]:
    name = item.get("name", "")
...
def refresh_cache(per_instrument_limit: Optional[int] = None):
